...
async def send_video_to_minio(user_id: int, message_id: int, filename: str, content_type: str, config: TelegramBotConfig) -> str:
    minio = await get_minio_instance(config)
    async with Client("bot", config.API_ID, config.API_HASH, bot_token=config.TELEGRAM_TOKEN) as app:
...
async def send_photo_to_minio(user_id: int, message_id: int, filename: str, content_type: str, config: TelegramBotConfig) -> str:
    minio = await get_minio_instance(config)
    async with Client("bot", config.API_ID, config.API_HASH, bot_token=config.TELEGRAM_TOKEN) as app:
        message = await get_message_by_id(app, user_id,  message_id)
        data = await app.download_media(message=message, in_memory=True)
