...
class ChatNamespace(AsyncNamespace):
    ...
    async def on_search(self, sid: str, data) -> None:
        ...
        if chat: return await self.emit(ERROR, {"message":"User in chat"})
        user = session["user"]
        ...
        for admin in admins:
            ...
            await bot.send_message(chat_id=admin.telegram_id, text=response, reply_markup=accept_chat(sid))
        await self.save_session(sid, session)
        await self.emit(SEACRH_STARTED, {"message":"Search started!", "searching":True}, to=sid)
